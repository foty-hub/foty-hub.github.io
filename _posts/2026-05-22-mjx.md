...
**Mission Un-complished**. Just reading a blog post, JAX looks pretty good. But it has a steep learning curve and [sharp edges](https://docs.jax.dev/en/latest/notebooks/Common_Gotchas_in_JAX.html). Additionally, MJX has some issues of its own: it typically runs at lower floating-point precision, and is [missing features](https://mujoco.readthedocs.io/en/stable/mjx.html#feature-parity) from normal MuJoCo. That said, the ability to run so many sims in parallel is enormously powerful, especially for data-hungry RL agents. Hopefully this has been a helpful whistle-stop tour around MJX and JAX!
